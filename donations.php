<?php 
    ...
    //get donations for the current page with the current sort
    record_set('contactlist',"SELECT * FROM donations, contacts WHERE donations.donor_id = contacts.contact_id $sorder $limit ");

    // record_set('donors', "SELECT * FROM contacts");

    // $donors_array = array();

    // $i = 1; do {  
    //       $donors_array[$i] = $row_donors['contact_first']; 
    // $i++; } while ($row_donors = mysql_fetch_assoc($donors)); 
 ?>
